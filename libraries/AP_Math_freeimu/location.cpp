...
{
    float scale = cosf(loc.lat * 1.0e-6f * DEG_TO_RAD);
...
/*void print_latlon(int32_t lat_or_lon)
{
    ...
    Serial.printf("%ld.%07ld",(long)dec_portion,(long)frac_portion);
}
*/
...
{
    Vector2d b1, b2, b3, q1, q2, q_diff;
    double ang;

    Vector2d A(0, 0);

    Vector2d Atmp(wpA.lat/1.0e7L,wpA.lng/1.0e7L);
    Vector2d B(wpB.lat/1.0e7L,wpB.lng/1.0e7L);
    Vector2d C(wpC.lat/1.0e7L,wpC.lng/1.0e7L);
    ...
    ang = acosf(-q1.x*q2.x-q1.y*q2.y)/2.0L;
    if(ang < 0.17L || ang > 1.39L)      //radians
    {
        return false;
    } else {
...
Vector2d geo2planar(Vector2d &ref, Vector2d &wp)
{
    ...
    Vector2d out;
    out.x=DEG_TO_RAD_DOUBLE*((wp.x-ref.x));
    out.y=DEG_TO_RAD_DOUBLE*((wp.y-ref.y)*cos(DEG_TO_RAD_DOUBLE*(ref.x)));
...
Vector2d planar2geo(Vector2d &ref, Vector2d &wp)
{
    //x-North (latitude), y-East (Longitude)
    Vector2d out;
    out.x=RAD_TO_DEG_DOUBLE*(wp.x)+ref.x;
    out.y=RAD_TO_DEG_DOUBLE*(wp.y*(1/cos(DEG_TO_RAD_DOUBLE*(ref.x))))+ref.y;
